  ...
  <div>
    <b-card>
      ...
      >
        ...
        >
          <b-form-checkbox
            v-for="field in taxAuctionFields"
            :key="field.key"
            v-model="fields"
            :value="field"
            name="flavour-3a"
...
import taxAuctionData from '@/views/import-tool/task-view/tax-auction'
import FormUpdate from '@/views/import-tool/task-view/tax-auction/FormUpdate.vue'
import FormFolios from '@/views/import-tool/task-view/tax-auction/FormFolios.vue'
...
export default {
  ...
  data() {
    return {
      fields: taxAuctionData.filter(item => item.active),
      items: [],
      perPage: 10,
      totalRows: 1,
      currentPage: 1,
  ...
  computed: {
    taxAuctionFields() {
      return taxAuctionData.filter(item => item.visible)
    },
  },
  async mounted() {
    await this.viewData()
