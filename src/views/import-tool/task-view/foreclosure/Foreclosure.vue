...
import FormUpdate from '@/views/import-tool/task-view/foreclosure/FormUpdate.vue'
import FormFolios from '@/views/import-tool/task-view/foreclosure/FormFolios.vue'
...
export default {
  ...
  data() {
    return {
      fields: foreclosureData.filter(item => item.active),
      items: [],
      perPage: 10,
      totalRows: 1,
      currentPage: 1,
  ...
  computed: {
    foreclosureFields() {
      return foreclosureData.filter(item => item.visible)
    },
  },
  async mounted() {
    await this.viewData()
