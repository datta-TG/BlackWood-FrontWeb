...
import probateData from '@/views/import-tool/task-view/probate'
import FormUpdate from '@/views/import-tool/task-view/probate/FormUpdate.vue'
import FormFolios from '@/views/import-tool/task-view/probate/FormFolios.vue'
...
export default {
  ...
  data() {
    return {
      fields: probateData.filter(item => item.active),
      items: [],
      perPage: 10,
      totalRows: 1,
      currentPage: 1,
  ...
  computed: {
    probateFields() {
      return probateData.filter(item => item.visible)
    },
  },
  async mounted() {
    await this.viewData()
